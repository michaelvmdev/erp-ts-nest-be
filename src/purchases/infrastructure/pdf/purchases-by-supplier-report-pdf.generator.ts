...
/**
 * Columnas: #, RUC (11 digitos fijos), Proveedor, Fecha, IGV, Monto.
 * No hay tipo de documento porque los proveedores solo usan RUC.
 */
const COLS = {
  item: MARGIN,  // #
  ruc: 76,       // RUC del proveedor
  supplier: 152, // Nombre del proveedor
  date: 356,     // Fecha de la compra
  igv: 416,      // IGV (derecha)
  amount: 476,   // Monto (derecha)
} as const;

@Injectable()
export class PurchasesBySupplierReportPdfGenerator
  implements PurchasesBySupplierReportPdfRenderer
{
  ...
  private encabezado(
    ...
    v: PurchasesBySupplierReportView,
  ): void {
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(11)
      .text('COMPRAS POR PROVEEDOR', bx, MARGIN + 8, {
        width: bw,
        align: 'center',
      });
    doc
    ...
    doc
      ...
      .fontSize(9)
      .text(
        `${v.totals.count} compra${v.totals.count === 1 ? '' : 's'}`,
        bx,
        MARGIN + 48,
        { width: bw, align: 'center' },
      );
    ...
    const titulo = v.singleDay
      ? `Compras por proveedor del dia ${v.dateFrom}`
      : `Compras por proveedor entre las fechas ${v.dateFrom} y ${v.dateTo}`;
    doc
      .fillColor(COLOR.accent)
      .font('Helvetica-Bold')
      .fontSize(11)
    ...
    doc
      ...
      .font('Helvetica')
      .fontSize(9)
      .text(`Proveedor: ${v.supplierDescription}`, MARGIN, 156, {
        width: RIGHT - MARGIN,
      });
  }
  ...
  private tabla(
    ...
    if (v.rows.length === 0) {
      doc
        ...
        .fontSize(9)
        .text('Sin compras del proveedor en el periodo.', MARGIN, y + 8, {
          width: RIGHT - MARGIN,
          align: 'center',
        });
      y += rowH + 8;
    ...
    const proveedorW = COLS.date - COLS.supplier - 6;
    ...
    v.rows.forEach((r, i) => {
      ...
      doc.text(String(i + 1), COLS.item + 2, ty, {
        width: COLS.ruc - COLS.item - 4,
      });
      doc.text(r.supplierRuc, COLS.ruc, ty, {
        width: COLS.supplier - COLS.ruc - 4,
        lineBreak: false,
      });
      doc.text(
      ...
      doc.text(r.purchaseDate, COLS.date, ty, {
