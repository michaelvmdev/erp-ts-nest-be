...
/** IGV y Monto a la derecha; el resto a la izquierda. */
const COLS = {
  item: MARGIN, // #
  docType: 70, // Tipo doc (DNI/RUC)
  docNumber: 102, // Nro doc
  client: 172, // Cliente
  date: 356, // Fecha
  igv: 416, // IGV (derecha)
  amount: 476, // Monto (derecha)
} as const;

/**
 * Dibuja el reporte "Ventas por Cliente" en un PDF: una fila por venta con el
 * documento del cliente, la fecha, el IGV y el monto.
 */
@Injectable()
export class SalesByClientReportPdfGenerator implements SalesByClientReportPdfRenderer {
  ...
  private encabezado(
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fontSize(11)
      .text('VENTAS POR CLIENTE', bx, MARGIN + 8, {
        width: bw,
        align: 'center',
      });
    doc
    ...
    doc
      ...
      .fontSize(9)
      .text(
        `${v.totals.count} venta${v.totals.count === 1 ? '' : 's'}`,
        bx,
        MARGIN + 48,
        { width: bw, align: 'center' },
      );
    ...
    const titulo = v.singleDay
      ? `Ventas por cliente del dia ${v.dateFrom}`
      : `Ventas por cliente entre las fechas ${v.dateFrom} y ${v.dateTo}`;
    doc
      .fillColor(COLOR.accent)
      .font('Helvetica-Bold')
      .fontSize(11)
  ...
  private cabeceraTabla(doc: PDFKit.PDFDocument, y: number): number {
    ...
    doc.text('Tipo', COLS.docType, ty);
    doc.text('Nro Doc', COLS.docNumber, ty);
  ...
  private tabla(doc: PDFKit.PDFDocument, v: SalesByClientReportView): number {
    ...
    const clienteW = COLS.date - COLS.client - 6;
    ...
    v.rows.forEach((r, i) => {
      ...
      doc.text(String(i + 1), COLS.item + 2, ty, {
        width: COLS.docType - COLS.item - 4,
      });
      doc.text(r.documentType, COLS.docType, ty, {
        width: COLS.docNumber - COLS.docType - 4,
        lineBreak: false,
      });
      doc.text(r.documentNumber, COLS.docNumber, ty, {
        width: COLS.client - COLS.docNumber - 4,
        lineBreak: false,
      });
      doc.text(
